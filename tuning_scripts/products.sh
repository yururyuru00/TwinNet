...
IFS=$'\n'
ary=("
     python3 train.py -m mlflow.runname=TwinSAGE_Products_L 'key=TwinSAGE_Products' \
     'TwinSAGE_Products.n_layer=2' \
     'TwinSAGE_Products.scope=choice(local,global)' \
     'TwinSAGE_Products.kernel=choice(wdp,ad)' \
     'TwinSAGE_Products.temparature=choice(-1,-0.5,0.5,1)' \
